...
class _AiSettingsScreenState extends State<AiSettingsScreen> {
  ...
  Future<void> _loadLastOk() async {
    final v = await AiClient.instance.getLastOkBaseUrl();
    if (!mounted) return;
    setState(() {
      _lastOk = v;
    });
  ...
  Future<void> _scan() async {
    setState(() {
      _testing = true;
      _testResult = 'Scanning...';
    });

    try {
      final candidate = _normalize(_ctrl.text);
      final found = await AiClient.instance.discoverAndSetBaseUrl(
        manualCandidateBaseUrl: candidate,
        port: 8010,
        probeTimeout: const Duration(milliseconds: 1200),
      );
      ...
      if (found == null) {
        setState(() {
          _testResult =
              'FAIL: no reachable endpoint found. Check USB/Wi-Fi and server.';
        });
        return;
      }
        ...
        children: [
          ...
          TextField(
            ...
            decoration: const InputDecoration(
              ...
              hintText: 'http://172.20.10.11:8010',
            ),
          ),
          ...
          Row(
            children: [
              Expanded(
                child: OutlinedButton(
                  onPressed: _testing ? null : _scan,
                  child: const Text('Scan (auto)'),
                ),
              ),
              const SizedBox(width: 12),
              Expanded(
